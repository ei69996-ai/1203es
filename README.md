...
- 환경별 Supabase 클라이언트 분리:
  ...
  - Server Component용 (`createClient` - async 패턴)
...
**📖 상세 설정 가이드**: [Clerk + Supabase 통합 설정 가이드](./docs/CLERK_SUPABASE_SETUP.md)를 참고하세요.
...
1. **Clerk Dashboard에서 Supabase 통합 활성화**
   ...
   - Clerk domain 복사 (예: `your-app.clerk.accounts.dev`)

2. **Supabase Dashboard에서 Clerk Provider 추가**
   - Supabase Dashboard → Authentication → Sign In / Up
   - "Add provider" → "Clerk" 선택
   - 위에서 복사한 Clerk domain 붙여넣기
   - Save 클릭

3. **RLS 정책 설정**
   - Supabase Dashboard → SQL Editor
   - `supabase/migrations/setup_rls_policies.sql` 파일 내용 실행

**추가 리소스:**
- [상세 설정 가이드](./docs/CLERK_SUPABASE_SETUP.md) - 단계별 상세 설명
...
**테스트 페이지:**
- `/instruments`: Supabase 공식 문서 예제 (Server Component 패턴)
- `/tasks`: 작업 목록 예제 (Clerk + Supabase 통합 모범 사례)
...
프로젝트에 이미 Clerk 한국어 로컬라이제이션이 적용되어 있습니다. `app/layout.tsx`의 `ClerkProvider`에서 커스텀 한국어 로컬라이제이션이 설정되어 있습니다.

**주요 특징:**
- 기본 한국어 로컬라이제이션 (`koKR`) 사용
- 커스텀 에러 메시지 지원
- 브랜드에 맞게 메시지 커스터마이징 가능
...
│   ├── instruments/     # Supabase 공식 문서 예제 페이지
│   ├── tasks/           # 작업 목록 예제 페이지
...
├── lib/                   # 유틸리티 및 설정
│   ├── clerk/             # Clerk 설정
│   │   └── localization.ts   # 한국어 커스텀 로컬라이제이션
...
│   │   ├── setup_rls_policies.sql # RLS 정책
│   │   └── setup_instruments_example.sql # 공식 문서 예제
...
### 프로젝트 문서
- [Clerk + Supabase 통합 설정 가이드](./docs/CLERK_SUPABASE_SETUP.md) - 상세한 단계별 설정 가이드
...
- [Supabase Next.js Quickstart](https://supabase.com/docs/guides/getting-started/quickstarts/nextjs) - 이 프로젝트가 따르는 공식 패턴
